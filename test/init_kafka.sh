...
# Build the protobuf regress producer the way PGXS builds the extension:
# take the compiler from pg_config (the exact toolchain this PostgreSQL was
# built with) instead of assuming a bare "gcc" is on PATH, and rely on
# PostgreSQL being built --with-openssl so plain -lssl -lcrypto resolve --
# no hard-coded, arch-specific library paths.  --start-group lets the linker
# resolve the libcurl.a <-> openssl interdependency regardless of order
# (needed when the only openssl available is a static archive).
...
"${CC}" -O2 -std=gnu99 \
    ...
    -Wl,--start-group \
    -lrdkafka -lssl -lcrypto -lprotobuf-c -lz -llz4 -lsasl2 -lzstd -lcurl \
    -Wl,--end-group \
    -lpthread -lrt -lm -ldl \
